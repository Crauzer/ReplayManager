...
/*
 * CFB added
 * for LoL CBC, CFB and ECB should only be needed.
 */
...
namespace ReplayManager.ROFL
{
    class BlowFish
    {
        ...
        public string Encrypt_CFB(string pt)
        {
            if (!IVSet)
                SetRandomIV();
            return ByteToHex(InitVector) + ByteToHex(Encrypt_CFB(Encoding.ASCII.GetBytes(pt)));
        }
        public string Decrypt_CFB(string ct)
        {
            IV = HexToByte(ct.Substring(0, 16));
            return Encoding.ASCII.GetString(Decrypt_CFB(HexToByte(ct.Substring(16)))).Replace("\0", "");
        }

        public byte[] Decrypt_CFB(byte[] ct)
        {
            return Crypt_CFB(ct, true);
        }

        public byte[] Encrypt_CFB(byte[] pt)
        {
            return Crypt_CFB(pt, false);
        }
        ...
        private byte[] Crypt_CFB(byte[] text, bool decrypt)
        {
            if (!IVSet)
            {
                throw new Exception("IV not set.");
